...
const URL_API2 : string = "http://localhost:9876/"; 
...
function TraerListadoProductoBD() {
    ...
    xhttp.open("GET", URL_API2 + "productos_bd", true);
        ...
        if (xhttp.readyState == 4 && xhttp.status == 200) {
            ...
            for (let index = 0; index < prod_obj_array.length; index++) {
                ...
                tabla += `<tr><td>${dato.codigo}</td><td>${dato.marca}</td><td>${dato.precio}</td>
                            <td><img src="${URL_API2}${dato.path}" width="50px" hight="50px"></td>
...
function AsignarManejadoresSeleccionProductoBD(){
    ...
    document.getElementsByName("btnSeleccionar").forEach((elemento)=>{
        ...
        elemento.addEventListener("click", ()=>{ ObtenerEliminar2(elemento)});
    });
}
...
function AgregarProductoBD() {
    ...
    xhttp.open("POST", URL_API2 + "productos_bd", true);
...
function ObtenerModificarProductoBD(dato:any) {
    ...
    (<HTMLImageElement>document.getElementById("imgFoto_m")).src = URL_API2 + obj_dato.path;
}
...
function ModificarProductoBD(){
    ...
    xhttp.open("POST", URL_API2 + "productos_bd/modificar", true);
...
function EliminarProductoBD(){
    ...
    xhttp.open("POST", URL_API2 + "productos_bd/eliminar", true);
...
function ObtenerEliminar2(dato:any) {
